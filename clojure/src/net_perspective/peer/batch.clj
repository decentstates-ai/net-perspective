...
(defn- collect-transitive-deps
  "For user-type relations in matching contexts, resolves CIDs and fetches deps.
   Returns [additional-hops source-cids]."
             ...
             (if-let [idx-cid (user-index-cid server rel-uid)]
               (try
                 (let [index    (fetch-document! server idx-cid #'schema/ContextRelationsDepsIndex)
                   ...
                   (if deps-cid
                     (let [deps (fetch-document! server deps-cid #'schema/ContextRelationsDeps)]
                       [(into hops (adjust-hops current-hop (get deps "crd/hops" [])))
                        (conj srcs deps-cid)])
                     [hops srcs]))
                 (catch Exception _ [hops srcs]))
               [hops srcs])))))
     [[] []]
     relations)))
            ...
            idx-ctxs (mapv
                      (fn [ctx]
                        (let [cpath   (get ctx "dr-ctx/path")
                              [extra-hops src-cids] (collect-transitive-deps server dr cpath 1)
                              hop1     {"crd-hop/hop"             1
                                        "crd-hop/dr-addresses"    [dr-cid]
                                        "crd-hop/archive-address" arch-cid
                                        "crd-hop/size"            (alength ^bytes dr-bytes)}
                              crd      (make-crd uid now cpath (into [hop1] extra-hops) src-cids)
                              crd-cid  (store-document! server crd #'schema/ContextRelationsDeps)
